...
class SettlementMonitor:
    ...
    def reconcile(self, client: object, account: MT5Account) -> int:
        ...
        for deal in deals:
            ...
            if order is None:
                continue
            if self._record_close(client, deal, order, deals):
                inserted += 1
    ...
    def _record_close(
        self, client: object, deal: object, order: object, history_deals: tuple[object, ...]
    ) -> bool:
        ...
        with connect_database(self.database_path) as database:
            cursor = database.execute(
                ...
                INSERT OR IGNORE INTO execution_close_events (
                    ...
                    swap,fee,net_profit,closed_at,created_at,updated_at
                )
                SELECT ?,g.mt5_account_id,?,?,?,?,?,?,?,?,?,?,?,?,?
                FROM execution_groups g WHERE g.id=?
                """,
                (user_id, group_id, order_id, ticket, reason, str(price), str(gross),
                 str(commission), str(swap), str(fee), str(net), closed_at, now, now, group_id),
            )
